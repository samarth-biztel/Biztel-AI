...
function CapabilityList({ items }: { items: string[] }) {
  return <div className="grid gap-x-6 gap-y-0 sm:grid-cols-2">{items.map(item => <div key={item} className="flex items-start gap-3 border-b border-line py-3.5"><Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-cyan-400" strokeWidth={1.8} /><span className="text-sm leading-5 text-steel-200">{item}</span></div>)}</div>;
}

export function Capabilities({ chapterNumber = '02' }: { chapterNumber?: string }) {
  return (
    <section id="capabilities" className="section-pad bg-surface">
