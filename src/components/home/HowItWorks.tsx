        ...
        <div className="mt-24 grid border border-line sm:grid-cols-2 lg:grid-cols-5">
          {steps.map((step, index) => (
            <Reveal key={step.name} delay={index * 0.08} className="h-full">
              <div className={`group relative flex h-full min-h-[355px] flex-col bg-muted p-10 transition-colors hover:bg-subtle ${index > 0 ? 'border-t sm:border-l sm:border-t-0' : ''} ${index === 2 ? 'sm:border-t lg:border-t-0' : ''}`}>
                ...
                <p className="mt-20 font-mono text-2xl font-bold uppercase tracking-[0.08em] text-ink">{step.name}</p>
